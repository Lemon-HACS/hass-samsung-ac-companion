...
로컬이 끊기면 로컬 엔티티만 unavailable 이 되고 클라우드 쪽은 계속 동작하므로,
둘을 함께 두는 것이 폴백이 된다.

**바람 방향과 바람문은 배타적이다.** 스탠드는 `Blooming`(상/중/하)으로 방향을
정하고 `wind.direction` 은 계속 `Off` 이므로, 유닛에 따라 `swing_mode` 의
선택지가 달라진다.
...
> 스탠드의 "닫힘"(`Blooming_0`)은 무풍 운전 중 기기가 스스로 만드는 상태라
> 선택지에 포함되어 있다.

### 그 밖에 (전부 로컬 API)

climate 로 표현할 수 없는 것들이다.

| 엔티티 | 기능 | 비고 |
|---|---|---|
| `switch.*_cheongjeong_dongsiunjeon` | 청정 동시운전 | 냉방/건조 + 청정 (**클라우드로 불가능**) |
| `switch.*_mudeudeung` | 무드등 | |
| `switch.*_jadongceongso` | 자동청소건조 | |
| `number.*_yeoldaeya_kwaemyeon` | 열대야 쾌면 | 0~12시간, 30분 단위 |
| `switch.*_barammun_{sang,jung,ha}` | 바람문 상/중/하 | 스탠드만. `swing_mode` 와 상태 공간은 같지만 한 곳만 여닫을 수 있다 |
| `sensor.*_sungan_jeonryeog` | 순간 전력(W) | 스탠드만 |

로컬 엔티티는 옵션에서 IP·토큰을 설정해야 활성화된다 (아래 참고).
...
**무풍 운전 중에는 바람문을 열 수 없다.** 기기가 `Blooming_N`(N≠0)을 받고도
`Blooming_0` 을 유지한다. 응답은 `204` 라 성공처럼 보이니 주의. 바람문을
바꾸려면 무풍(`Comode_Nano`)을 먼저 해제해야 한다.
