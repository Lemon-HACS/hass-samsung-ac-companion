...
- **전원(`/operation`)은 보통 5~6초, 길게는 수십 초 이상 걸립니다.** 2초 시점에 읽으면 켜지는 중인데도 `Off`가 돌아옵니다.
- 온도·풍량·옵션은 2초 안에 반영됩니다.

전원을 한 번 읽고 확정하면 실제로는 켜졌는데 UI가 "꺼짐"으로 되돌아갑니다. 사용자가 안 켜진 줄 알고 다시 누르면 토글이라 이번에는 진짜로 꺼집니다.

반영 시간이 들쭉날쭉해서 재조회 횟수를 늘리는 것으로는 해결되지 않습니다. 코디네이터는 대신 기기가 따라올 때까지 지시한 값을 유지하고, 3분 안에 반영되지 않으면 기기가 거부한 것으로 보고 실제 값을 보여줍니다.
